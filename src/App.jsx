...
import { Budgets } from './modules/budgets/Budgets.jsx';
import { Debts } from './modules/debts/Debts.jsx';
import { Savings } from './modules/savings/Savings.jsx';
import { AuthGate } from './modules/auth/AuthGate.jsx';
import { Settings } from './modules/settings/Settings.jsx';

// Hooks y utils
import useHouseholdData from './hooks/useHouseholdData.js';
import {
  monthKey,
  shiftMonth,
...
// Config / estilos globales (side-effects)
import './App.css';
...
export default function App() {
  ...
  // Persistir usuario de la casa seleccionado
  useEffect(() => {
  ...
  // Estado de autenticación Firebase (email/contraseña)
  useEffect(() => {
  ...
  const budgetsForSelectedMonth = (h.budgets && h.budgets[selectedMonth]) || {};
  const householdUsers = h.householdUsers || [];
  const activeUser =
    householdUsers.find((u) => u.id === selectedHouseholdUserId) || null;

  // Lógica de selector de usuario de la casa (PIN / admin)
  useEffect(() => {
    ...
    // Si el admin no tiene PIN, no obligamos selector
    if (!superHasPin) {
      setShowUserSelector(false);
    ...
    // Si ya se forzó selección antes
    if (hasForcedUserSelection) {
      ...
      if (!exists) {
        ...
        setLoginUserId((superAdmin && superAdmin.id) || householdUsers[0].id);
        setLoginPin('');
        setLoginError('');
      }
      return;
    ...
    // Primera vez: obligamos a elegir usuario
    setShowUserSelector(true);
  ...
  async function handleUserLogin(e) {
    ...
    if (!useAccountPassword) {
      ...
      if (!user.isSuperAdmin) {
        setLoginError(
          'Solo el administrador puede usar la contraseña de la cuenta.',
        );
        return;
      }
  ...
  const actions = {
    ...
    setBudgetFunded: (monthKeyStr, cat, value) =>
      h.setBudgetFunded(monthKeyStr, cat, value),
  ...
  const superAdminUser = householdUsers.find((u) => u.isSuperAdmin) || null;
  const superAdminHasPin = !!(
    superAdminUser &&
    superAdminUser.pin &&
    superAdminUser.pin.trim()
  );
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 w-full max-w-md text-gray-900 dark:text-gray-100">
            ...
            {householdUsers.length === 0 ? (
              <p className="text-sm text-gray-600">
                No hay usuarios configurados aún. Ve a Ajustes &gt; Usuarios de
                la casa para crearlos.
              </p>
            ) : (
              <form
                ...
                <div className="grid gap-2">
                  {!useAccountPassword && (
                    <div className="grid gap-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Si olvidaste tu PIN, pídele ayuda al administrador del
                        hogar.
                      </p>
                    </div>
                  )}
                  ...
                  {useAccountPassword && (
                    <div className="grid gap-1">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Usaremos tu contraseña de Firebase para confirmar que
                        eres el administrador y dejarte entrar aunque no
                        recuerdes el PIN.
                      </p>
                    </div>
                  )}
                  ...
                  {(() => {
                    ...
                    if (!selectedUser || !selectedUser.isSuperAdmin)
                      return null;
                    return (
                      <button
                        type="button"
